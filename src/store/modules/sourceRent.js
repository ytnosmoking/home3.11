...
export default {
  ...
  mutations: {
    ...
    guImportanceTypeName(state, payload) { // 重视类型 重环境 重交通  重租金
      state.guImportanceTypeName = payload.value
    },
    guImportanceTypeId(state, payload) { // 重视类型 ID
      state.guImportanceTypeId = payload
    },
  ...
  actions: {
    getTable({ commit, state }, tableInfo) {
      ...
      return new Promise((resolve, reject) => {
        ...
        console.log(state)
        const params = Object.assign({}, state, { userid: getItem("userid") })
        const data = Object.assign({}, { params })
        // console.log(data)
